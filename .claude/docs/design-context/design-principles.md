...
Use the following as guiding principles. Use your judgement to decide which which of the following pillars are applicable in a given project. If its a mobile app only project, use ONLY the pointers which are applicable for mobile. **IF a project already have some of the foundation work done, e.g. typogrpahy or color is defined, then DO NOT overwrite the design system**. 
